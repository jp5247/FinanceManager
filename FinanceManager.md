...
- **Canonical category list** (P1 resolved): 26 user-curated labels — bills (Credit card bill, Electricity bill, Gas bill, Mobile/Internet bill, Laundary bill), EMIs (Home Loan EMI, Car loan EMI, CC EMI), lifestyle expenses (Food expenses, Hotel/Vacation expenses, Fuel expenses, Vehicle repairs/maintenance, Medical expenses, Groceries, Transportation, Personal care, Shopping, Entertainment, Gifts, Split), income (Salary, Side Hustle, Interest), wealth-building (SIP, Stock purchase, FD) — plus four system categories (Refund, Dividend, Bank Transfer, EMI Conversion, Uncategorized). Source of truth in `src/categories.ts`; mirrored in the LLM's `ALLOWED_CATEGORIES`. Curated merchant rules + LLM prompt updated to use these names.
- **Split category for reimbursable expenses**: when the user pays the whole bill for a group and expects reimbursement later, both the initial outlay (debit) and the friend's reimbursement (credit) are categorized `Split`. Per-category netting then yields the user's actual share as net expense — fully settled splits net to zero, partially settled splits leave the user's true share as expense. Pinned by `split_category_nets_reimbursements_against_initial_outlay` and `split_category_fully_settled_nets_to_zero_expense`. Implements decision P2.
- **Reset categories**: new `Reset categories` button in the user-rules panel triggers `reset_categorizations`, which deletes every user rule, empties the merchant cache, and re-categorizes every import from scratch (also dropping manual edits — intentional, the user asked for a clean slate). UI confirms before proceeding. Touched / skipped counts returned.
- **Credit-card EMI handling**: when a purchase is converted to EMI on a credit card, statements typically surface three related rows — the original purchase debit, the loan principal being booked (a second debit), and the loan disbursement (a credit). The new **`EMI Conversion`** category (classified as Transfer kind in `dashboard.rs`) keeps these bookkeeping rows out of both income and expense so only the actual recurring monthly installment (`Loan EMI`), processing fee (`Bills`), and GST (`Tax`) count as real outflow. Curated rules auto-match HDFC patterns: `aggregator-emi`, `offuscredit`, `emi booking`. The LLM prompt was also updated to recognise the same semantic. The original purchase debit (which has no auto-detectable EMI marker) still needs a one-time recategorize from the drill modal — saving the rule applies it retroactively across statements.
...
- Audit-write call sites (best-effort — errors swallowed to stderr so a missing audit file never blocks a user action):
  - `upload_pdf` (action `upload_pdf`)
  - `recategorize_transaction` (action `recategorize_transaction`, includes rule-saved details when applicable)
  - `delete_import` (action `delete_import`)
  - `reset_categorizations` (action `reset_categorizations` with total/touched/skipped counts)
  - `upsert_investment` (action `create_investment` or `update_investment`)
  - `delete_investment` (action `delete_investment`)
  - `upsert_loan` (action `create_loan` or `update_loan`)
  - `delete_loan` (action `delete_loan`)
...
- ✅ Past Analysis (Section 10.8)
- ✅ Investment + Loan modules feed analysis (Sections 10.6, 10.7)
- ✅ Excel export (Section 10.9)
- ✅ Multi-user profile separation (Section 10.2)
- ✅ No sensitive data leaves the device (OD-5 egress guard, Section 10.4)
- ✅ Hash-chained audit log surfaced in UI (Section 10.10)
