...
| ~~Trend negative-OUT bug~~ | Monthly expense could go negative when refunds on expense-categorized rows dominated debits in that month (visible as `−₹X` in the OUT row and a full-width bar via CSS fallback). Now: per-row cash-flow direction wins — a debit on an expense category goes to expense, a credit goes to income. Same applies to investment-kind rows (credits → income, e.g. mutual-fund payouts). Pinned by `credit_on_expense_category_lands_in_income_not_negative_expense`. Bar widths additionally `clampPct`ed to `[0, 100]` as defense-in-depth. | (this commit) |
